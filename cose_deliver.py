...
function_list = {}
bo_release_list = []
bo_plat_resource = Platform_Resource()
...
def release_resource(request_time):
    global bo_release_list
    if len(bo_release_list) > 0:
        bo_release_list = sorted(
            bo_release_list, key=lambda i: i['release_time'], reverse=False)

        while request_time > bo_release_list[0]['release_time']:
            bo_plat_resource.add_resource(
                bo_release_list[0]['platform_name'], bo_release_list[0]['node'], bo_release_list[0]['cpu'], bo_release_list[0]['memory'])
            bo_release_list.pop(0)
...
def BO(function, parameter, logger, function_list, request_time, iteration):
    function_name = function + "_" + parameter
    slo = function_list[function_name]["slo"]
    device_edge_max, cloud_edge_max, cloud_max = bo_plat_resource.get_resource()
    # cost = 0.0
    if function_list[function_name]["iteration"] == 0:
        pbounds = {'platform': (1.0, 3.999999), 'cpu_config': (
        ...
        function_list[function_name]["optimizer"] = optimizer
        function_list[function_name]["observered"] = []

    utility = UtilityFunction(kind="ei", kappa=2.5, xi=0.0)

    release_resource(request_time)
    next_point = function_list[function_name]["optimizer"].suggest(utility)
    ...
    if next_point not in function_list[function_name]["observered"] and consume_success:
        ...
        function_list[function_name]["optimizer"].register(
    ...
    if function_list[function_name]["iteration"] == iteration:
        logger.info("BO result: platform: %s, cpu: %s, memory %s" % (get_platform_name(int(function_list[function_name]["optimizer"].max['params']['platform'])), get_cpu(
            int(function_list[function_name]["optimizer"].max['params']['cpu_config'])), get_memory(int(function_list[function_name]["optimizer"].max['params']['memory_config']))))
        logger.info("BO result: max: %s" %
                    (function_list[function_name]["optimizer"].max['target']*-1))

        return function_list[function_name]["optimizer"].max['target'] * -1
    return 0
